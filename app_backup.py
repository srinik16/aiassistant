...
def _create_sequential_chain() -> Optional[SequentialChain]:f m.type == "human" else "assistant")
...
def _get_retriever(k: int = 4):ional[ChatOpenAI]:
    """Return a cached ChatOpenAI instance if API key is available."""
    api_key = os.getenv("OPEN_AI_API_KEY")
    if not api_key:
        return None
...
def build_prompt(question: str, contexts: Sequence[dict]) -> list:ata
...
def _render_doc_pills() -> None:t relevant chunks
...
def _render_chat_area() -> None:g.content if hasattr(msg, 'content') else str(msg)
